    ...
    return ({ context, width, height }) => {
       ...
       for (let i = 0; i < numCircles; i++) {
            ...
            for(let j = 0; j < numSlices; j++ ) {
                ...
                bin = bins[i * numSlices * j] //primera parte es para cada circulo y segunda para cada porcion de cada circulo
                if(!bin) continue;
...
const addListeners = () => {
    window.addEventListener("mouseup", () => {
        if (!audioContext) createAudio(); //solo se crea una vez
        if (audio.paused) {
            audio.play();
            manager.play();
        } else {
...
const createAudio = () => {
    //play audio on a browser
    audio = document.createElement("audio");
    audio.src = "./audio/FrozenjaZz - Hold Me.mp3";
